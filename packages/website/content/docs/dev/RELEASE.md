...
The release pipeline is triggered by pushing a `v*` tag. The `Release MDComment` workflow (`.github/workflows/release.yml`) then runs **validate → build (5-platform matrix) → publish** and creates a GitHub Release with installers and `SHA256SUMS.txt`.
